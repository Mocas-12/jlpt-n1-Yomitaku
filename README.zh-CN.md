...
- ✅ **内置题库**：18 组 35 问，覆盖全部六大题型，为参照官方「大題的测试目标」（字数、能力点、设问方式）编写的**原创模拟题**，每组明确标注「非真题」
...
站点本体保持零依赖；`package.json` 仅用于开发期冒烟测试（Playwright，覆盖训练完整链路、混合训练与模拟卷、未答完提交确认、导入校验正反例、深色模式、键盘作答、记录备份、错题重练闭环、PWA 离线与数据养护）。CI 会在每次部署前自动跑一遍。
